...
-- ORGS: members can read their org; admins can update
...
-- ORG_MEMBERSHIPS: members see their org's memberships; no cross-org
...
CREATE POLICY usage_events_owner_rls ON usage_events
  ...
  WITH CHECK (account_id = current_setting('app.current_user_id', true)::uuid);
...
CREATE POLICY embeddings_owner_rls ON vectors.embeddings
  USING (
    (owner_scope = 'global')
    OR (owner_scope = 'user' AND owner_id = current_setting('app.current_user_id', true)::uuid)
    OR (owner_scope = 'org' AND owner_id = current_setting('app.current_org_id', true)::uuid)
  );
